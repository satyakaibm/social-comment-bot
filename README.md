...
Each cron cycle is bounded to recent content and a fixed number of comments per platform. Edit `config/polling.env` to control how many YouTube videos, Facebook posts, Instagram media items, and comments are checked in one run. The three `*_PUBLISH_LIMIT` values control how many pending replies can be attempted in that cycle. `PUBLISH_ERROR_LIMIT` stops a platform after repeated consecutive API errors. The script also prevents overlapping cron runs and handles interruption without a Python traceback. The cron entry does not need limit variables:
